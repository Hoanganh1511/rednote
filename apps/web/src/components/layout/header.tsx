...
const RIGHT_ACTIONS = [{ label: 'Tin nhắn', href: '/messages', icon: MessageCircle }] as const;
    ...
    <div className="relative shrink-0" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
          ...
          <a href="/collection" className="text-xs text-[#00aeec] hover:underline transition-opacity hover:opacity-80">
            Xem tất cả →
          </a>
        </div>
        ...
        <div className="flex">
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-3 py-16">
            ...
            <p className="text-sm text-muted-foreground">Chưa có video nào trong bộ sưu tập này</p>
          </div>
        </div>
      </div>
    </div>
      ...
      >
        ...
        >
          <div className="flex flex-col items-center gap-2.5 px-5 pt-14 pb-6">
            <p className="text-base font-semibold">{user.displayName ?? user.username ?? 'Người dùng'}</p>
            {user.phoneNumber && <p className="text-muted-foreground text-xs">{user.phoneNumber}</p>}
